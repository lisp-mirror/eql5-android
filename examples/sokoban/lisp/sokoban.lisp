...
(defun connect ()
  (macrolet ((clicked (item function)
               `(qconnect (find-quick-item ,item) "clicked()" (lambda () ,function))))
    (clicked "up"       (sokoban:move :north *maze*))
    (clicked "down"     (sokoban:move :south *maze*))
    (clicked "left"     (sokoban:move :west *maze*))
    (clicked "right"    (sokoban:move :east *maze*))
    (clicked "previous" (change-level :previous))
    (clicked "next"     (change-level :next))
    (clicked "undo"     (undo))
    (clicked "restart"  (reset-maze))
    (clicked "solve"    (solve))))
